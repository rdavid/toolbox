...
It removes a torrent with content and then adds it again.
It can be run by `cron` to increase a ratio.
* link:app/ydata[`ydata`] is a wrapper around {url-yt-dlp}[`yt-dlp`]: downloads,
converts, renames and keeps safe. It can be run by `cron`.
