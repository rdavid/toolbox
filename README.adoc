...
and operates within the Unix shell framework {url-shellbase}[`shellbase`]:
...
* link:app/scanner[`scanner`] detects whether a device with a certain MAC
address is present in the local network, uses {url-arp-scan}[`arp-scan`].
...
It removes a torrent and its content, then adds it again.
It can be run by `cron` to increase a ratio.
* link:app/ydata[`ydata`] wraps {url-yt-dlp}[`yt-dlp`]: downloads, converts,
renames, and stores safely. It can be run by `cron`.
